...
export const getGameCounts = createAsyncThunk(
  "nepalGameCount/getAll",
  async (_, { rejectWithValue }) => {
    try {
      const { data } = await api.get("/admin/nepal/game-count");
      return data.data;
    } catch (err) {
...
export const getGameCount = createAsyncThunk(
  "nepalGameCount/getOne",
  async (id, { rejectWithValue }) => {
    try {
      const { data } = await api.get(`/admin/nepal/game-count/${id}`);
      return data.data;
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || err.message);
    }
...
export const createGameCount = createAsyncThunk(
  "nepalGameCount/create",
  async (payload, { rejectWithValue }) => {
    try {
      const { data } = await api.post("/admin/nepal/game-count", payload);
      return data.data;
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || err.message);
    }
...
export const updateGameCount = createAsyncThunk(
  "nepalGameCount/update",
  async ({ id, formData }, { rejectWithValue }) => {
    try {
      const { data } = await api.put(`/admin/nepal/game-count/${id}`, formData);
      return data.data;
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || err.message);
    }
...
export const deleteGameCount = createAsyncThunk(
  "nepalGameCount/delete",
  async (id, { rejectWithValue }) => {
    try {
      await api.delete(`/admin/nepal/game-count/${id}`);
      return id;
    } catch (err) {
      return rejectWithValue(err.response?.data?.message || err.message);
    }
