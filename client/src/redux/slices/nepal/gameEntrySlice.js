...
export const createGameEntry = createAsyncThunk(
  "nepalGameEntry/create",
  async (formData, { rejectWithValue }) => {
    try {
      const { data } = await api.post("/nepal/game-entry", formData);
      return data;
    } catch (error) {
...
export const getMyGameEntries = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      ...
      const url = queryString ? `/nepal/game-entry?${queryString}` : "/nepal/game-entry";
...
export const getSingleGameEntry = createAsyncThunk(
  "nepalGameEntry/getSingle",
  async (id, { rejectWithValue }) => {
    try {
      const { data } = await api.get(`/nepal/game-entry/${id}`);
      return data.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Something went wrong."
...
export const getEntryResults = createAsyncThunk(
  "nepalGameEntry/getResults",
  async (id, { rejectWithValue }) => {
    try {
      const { data } = await api.get(`/nepal/game-entry/${id}/results`);
      return data.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Something went wrong."
...
export const deleteGameEntry = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      await api.delete(`/nepal/game-entry/${id}`);
      return id;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Something went wrong."
...
export const cancelGameEntry = createAsyncThunk(
  "nepalGameEntry/cancel",
  async (id, { rejectWithValue }) => {
    try {
      const { data } = await api.put(`/nepal/game-entry/${id}/cancel`);
      return data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Something went wrong."
...
export const getUserBalance = createAsyncThunk(
  "nepalGameEntry/getBalance",
  async (_, { rejectWithValue }) => {
    try {
      const { data } = await api.get("/nepal/game-entry/balance/me");
      return data.balance;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Something went wrong."
...
export const getEntriesByCountry = createAsyncThunk(
  "nepalGameEntry/getByCountry",
  async (country, { rejectWithValue }) => {
    try {
      const { data } = await api.get(`/nepal/game-entry/country/${country}`);
      return data.data;
    } catch (error) {
      return rejectWithValue(
        error.response?.data?.message || "Something went wrong."
...
const nepalGameEntrySlice = createSlice({
  name: "nepalGameEntry",
  initialState,
  reducers: {
...
export const { 
  ...
  clearFilters,
  updatePagination
} = nepalGameEntrySlice.actions;

// ================= SELECTORS =================
export const selectAllEntries = (state) => state.nepalGameEntries.entries;
export const selectSelectedEntry = (state) => state.nepalGameEntries.selectedEntry;
export const selectEntryResults = (state) => state.nepalGameEntries.results;
export const selectWinningNumbers = (state) => state.nepalGameEntries.winningNumbers;
export const selectBalance = (state) => state.nepalGameEntries.balance;
export const selectGameEntryLoading = (state) => state.nepalGameEntries.loading;
export const selectGameEntrySuccess = (state) => state.nepalGameEntries.success;
export const selectGameEntryError = (state) => state.nepalGameEntries.error;
export const selectGameEntryMessage = (state) => state.nepalGameEntries.message;
export const selectPagination = (state) => state.nepalGameEntries.pagination;
export const selectFilters = (state) => state.nepalGameEntries.filters;
export const selectEntriesByCountry = (state, country) => 
  state.nepalGameEntries.entries.filter(entry => entry.country === country);
export const selectEntriesByStatus = (state, status) => 
  state.nepalGameEntries.entries.filter(entry => entry.poolStatus === status);

export default nepalGameEntrySlice.reducer;