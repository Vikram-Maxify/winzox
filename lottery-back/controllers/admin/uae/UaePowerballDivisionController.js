...
const UaePowerballDivision = require("../../../models/uae/UaePowerballDivision");
...
exports.createDivision = async (req, res) => {
  try {
    ...
    const existingDivision = await UaePowerballDivision.findOne({
    ...
    const newDivision = await UaePowerballDivision.create({
      division: Number(division),
      main: Number(main),
      powerball,
      prize: Number(prize),
    ...
    return res.status(201).json({
      success: true,
      message: "UAE Powerball division created successfully.",
      division: newDivision,
    });
  } catch (error) {
    console.error("Create UAE Powerball Division Error:", error);
...
exports.getAllDivisions = async (req, res) => {
  try {
    const divisions = await UaePowerballDivision.find().sort({ division: 1 }).lean();
    ...
    console.error("Get UAE Powerball Divisions Error:", error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.getActiveDivisions = async (req, res) => {
  try {
    const divisions = await UaePowerballDivision.find({ isActive: true })
      .sort({ division: 1 })
      .lean();
    ...
    console.error("Get Active UAE Powerball Divisions Error:", error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.getDivisionById = async (req, res) => {
  try {
    ...
    const division = await UaePowerballDivision.findById(id).lean();

    if (!division) {
      return res.status(404).json({
        success: false,
        message: "UAE Powerball division not found.",
      });
    }
    ...
    console.error("Get UAE Powerball Division Error:", error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.updateDivision = async (req, res) => {
  try {
    ...
    const existing = await UaePowerballDivision.findById(id);
    ...
    if (
      ...
      const duplicate = await UaePowerballDivision.findOne({
        division: Number(division),
        _id: { $ne: id },
      });
    ...
    return res.status(200).json({
      success: true,
      message: "UAE Powerball division updated successfully.",
      division: existing,
    });
  } catch (error) {
    console.error("Update UAE Powerball Division Error:", error);
...
exports.deleteDivision = async (req, res) => {
  try {
    ...
    const division = await UaePowerballDivision.findById(id);
    ...
    return res.status(200).json({
      success: true,
      message: "UAE Powerball division deleted successfully.",
    });
  } catch (error) {
    console.error("Delete UAE Powerball Division Error:", error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
...
exports.toggleDivisionStatus = async (req, res) => {
  try {
    ...
    console.error("Toggle UAE Powerball Division Error:", error);
    return res.status(500).json({
      success: false,
      message: error.message,
    });
